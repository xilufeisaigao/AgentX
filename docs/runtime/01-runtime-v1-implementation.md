...
2. 本文回答的是“当前代码到底已经实现到哪”，因此更适合作为实现基线和对照归档。
...
1. 这是当前代码真相。
2. 目标设计文档已经把后续主链升级为 `merge-gate -> integration-test-gate -> verify`，也就是 verify agent 延后到模块级集成测试之后再启动。
3. 该升级尚未落代码，因此本文继续按当前实现描述，不把目标设计写成已实现事实。
...
11. 本地 lexical / symbol RAG 与 repo / overlay index 基线（当前真相）
...
1. 当前代码里，merge 成功后会直接进入 verify。
2. 目标设计已调整为“merge-gate 先沉淀模块集成候选，verify agent 只在模块级集成测试之后启动”。
...
1. 这里描述的是当前代码路径，即 task merge 成功后立即 verify。
2. 目标设计下，这一段会改成“模块集成测试闸门先跑确定性集成测试，再启动 verify agent 做模块级裁决”。
...
后续接真实 LLM、Unix 探索式 coding、K8s 或控制面时，应该继续沿：
