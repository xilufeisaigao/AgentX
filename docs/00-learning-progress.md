...
3. 只有当一段解释确实比较复杂、概念密集或理解门槛较高时，后面才补完整中文翻译。
4. 普通段落不要整段翻译，只保留英文主讲和术语括注。
5. Mermaid 图里的关键标签也要补中文提示。
6. 目标不是华丽英文，而是清晰、可学、便于你练习的工程英语。
...
- 今日流程图主题: Docker 运行面 -> 运行时配置真相 -> 真实闭环样本 -> 总链路入口
- 当前微轮次: R1
- 当前 checkpoint: Docker 运行面与运行时 LLM 配置真相
- 下一轮目标: 继续读最小闭环样本证据，再进入 [03-end-to-end-chain.md](architecture/03-end-to-end-chain.md)
- 当前等待动作: 用户发出 `继续学习`、`继续`，或针对本轮运行面 / 配置覆盖机制提问
...
| 2026-03-21 | R1 | 运行面与配置真相 | Docker 栈入口、运行时 LLM 配置覆盖、为什么 `.env.docker` 不是最终真相 | yes | 暂无 | R2: 最小闭环样本与进入主链路 |
